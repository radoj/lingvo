...
A sintaxe compacta permite duplicar o contexto utilizando uma lista de valores que é associado em ordem aos elementos declarados. Cada contexto terá então uma lista de elementos para serem utilizados na sintaxe compacta.

Efeito similar ao acionamento de construtores de instâncias da classe.
...
<1> f1 é subcontexto de Equacao, porém com a lista sem os elementos a e b diferente x,y

==== Definindo o comportamento do operador ()

É possível fazer com que o operador () não retorne um subcontexto, mas sim o valor de um vocábulo do subcontexto, através da declaração
...
No entanto seria melhor se isso pudesse já ser definido no supercontexto, evitando a repetição:
